...
public class SoundSearchTIle extends QSTileImpl<BooleanState> {
    ...
    public void handleClick() {
        ...
        // Shazam
        if (LotusUtils.isPackageInstalled(mContext, "com.shazam.android") || LotusUtils.isPackageInstalled(mContext, "com.shazam.encore.android")) {
            Intent intent = new Intent(Intent.ACTION_MAIN);
            intent.setAction("com.shazam.android.intent.actions.START_TAGGING");
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
            mContext.startActivity(intent);
        // Soundhound
        } else if (LotusUtils.isPackageInstalled(mContext, "com.melodis.midomiMusicIdentifier.freemium") || LotusUtils.isPackageInstalled(mContext, "com.melodis.midomiMusicIdentifier")) {
            Intent intent = new Intent(Intent.ACTION_MAIN);
            intent.setAction("com.soundhound.android.ID_NOW_EXTERNAL");
            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
            mContext.startActivity(intent);
        // Google Search Music
        } else if (LotusUtils.isPackageInstalled(mContext, "com.google.android.googlequicksearchbox")) {
            Intent intent = new Intent(Intent.ACTION_MAIN);
